...
switch ($_GET['option']) {
    ...
    case 'dividir':
        $tipo_caso = isset($_GET['tipo_caso']) ? $_GET['tipo_caso'] : null;
        
        $data = $group_status['casos_para_dividir'][$tipo_caso];
        print_r($data);
        /* foreach ($data as $caso){
            print_r(dividirPorCaso($caso));
            die();
        } */
